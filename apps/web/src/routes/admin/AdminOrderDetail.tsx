...
import { useAdminOrderDetail, useAdminUpdateOrderStatus, useAdminAddAWB, useShipOrder, ShipOrderParams } from '@/features/orders/hooks';
...
export default function AdminOrderDetail() {
  ...
  const isBusy = updateStatus.isPending || addAWB.isPending || shipOrder.isPending;

  return (
    <div className="p-4 md:p-6">
      <div className="mb-6">
        <Link to="/admin/orders" className="text-sm text-gray-500 hover:text-brand-accent">&larr; Back to Orders</Link>
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mt-2">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <span className={`px-3 py-1 text-sm ${order.paymentStatus === 'PAID' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{order.paymentStatus}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
          ...
          <div className="bg-white p-4">
            <h3 className="font-bold mb-3">Publisher</h3>
            <p className="text-sm">{order.publisher?.fullName || '—'}</p>
          </div>
          ...
          <div className="bg-white p-4">
            ...
            {order.items?.map((item: any) => (
              <div key={item.id} className="flex justify-between text-sm py-2 border-b last:border-0">
                <span className="pr-2">{item.productNameSnapshot} &times; {item.quantity}</span>
                <span className="whitespace-nowrap">Rp {Number(item.subtotalMarketplacePrice).toLocaleString()}</span>
              </div>
            ))}
            <div className="flex justify-between text-sm font-bold pt-2 border-t mt-2">
              <span>Grand Total</span>
              <span>Rp {Number(order.grandTotal).toLocaleString()}</span>
            </div>
          </div>
          ...
          <div className="bg-white p-4">
            ...
            <p className="text-sm text-gray-600">{order.shippingAddress?.fullAddress || order.shippingAddress?.address || '—'}</p>
            {order.shipment ? (
              <>
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={!awbNumber || isBusy}
                    className="bg-brand-accent text-white px-4 py-2 text-sm disabled:opacity-50 rounded-sm"
                  >
        ...
        <div className="space-y-6">
          <div className="bg-white p-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {['PENDING_PAYMENT', 'PAID', 'DELIVERED', 'CANCELLED'].map((s) => (
                <button
                  key={s}
                  onClick={() => updateStatus.mutate({ id: id!, status: s })}
                  disabled={order.orderStatus === s || isBusy}
                  className={`py-2 text-sm rounded-sm disabled:opacity-50 ${
                    order.orderStatus === s ? 'bg-gray-200 text-gray-500 cursor-not-allowed' :
            ...
            {(order.orderStatus === 'PAID' || order.orderStatus === 'PROCESSING') && !order.shipment?.awb && (
              <button
                ...
                disabled={isBusy}
                className="w-full mt-3 bg-orange-500 text-white py-2 text-sm rounded-sm disabled:opacity-50"
              >
                {shipOrder.isPending && <Spinner />}
                {shipOrder.isPending ? 'Mengirim...' : 'PROCESS'}
              </button>
            )}
          </div>
          ...
          <div className="bg-white p-4">
            ...
            {order.payment ? (
              <div className="text-sm space-y-1">
                ...
                <p>Amount: Rp {Number(order.payment.amount).toLocaleString()}</p>
                <p>Status: <span className={`px-2 py-1 text-xs ${order.payment.status === 'PAID' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{order.payment.status}</span></p>
              </div>
            ) : (
