...
export default function NotificationBell() {
  ...
  return (
    <div className="relative">
      <button onClick={() => setIsOpen(!isOpen)} className="relative p-2 text-gray-400 hover:text-white rounded-sm">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-white shadow-lg z-50 max-h-96 overflow-y-auto">
          <div className="flex justify-between items-center p-3 border-b">
            ...
            {unreadCount > 0 && (
              <button onClick={() => markAllRead.mutate()} className="text-xs text-brand-accent rounded-sm">Mark all read</button>
            )}
          </div>
